 ...
 * address is validated, normalized, and reconciled against the account's signing
 * key, so a re-wrapped account whose reported address diverges from its key fails
 * at construction instead of silently signing for the wrong account.
...
export async function createSigner(
  ...
  const account = toAccount({
    address: normalizeAddress(privyViemAccount.address, 'address'),
